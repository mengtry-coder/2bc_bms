...
class GlobalFunction 
{
    ...
    public static  function SettingValue($id)
    {    
        return Yii::$app->db->createCommand("select value from setting where id = $id and status = 1")->queryScalar();  
    }

    public static function AutoCorrectChartOfAccount($account_type, $chart_of_account){
        $data_chart_of_account = \backend\models\FinanceChartOfAccount::find()
            ->select(['id','debit', 'credit', 'balance'])
            ->where(['account_type'=>$account_type, 'chart_of_account'=>$chart_of_account, 'status'=>1])
            // ->orderBy(['created_date'=>SORT_ASC, 'date'=> SORT_ASC])
            ->orderBy(['date'=>SORT_ASC, 'id'=>SORT_ASC])
            ->all();

        $update_balance = 0;
        if(!empty($data_chart_of_account)){
            if($data_chart_of_account[0]['credit'] > 0){
                // If Index Start as Credit
                foreach ($data_chart_of_account as $key => $value) {
                    if($key == 0){ //Update Starting Index
                        $update_balance = $value['credit'] - $value['debit'];
                        $value->balance = $update_balance;
                        $value->update();
                        ...
                        if(!isset($update_balance)){
                            ...
                            $update_balance = $update_balance + $value['credit'] - $value['debit'];
                            $value->balance = $update_balance;
                            $value->update();
                        }
                    }
                ...
                // If Index Start as Debit
                foreach ($data_chart_of_account as $key => $value) {
                    if($key == 0){ //Update Starting Index
                        $update_balance = $value['debit'] - $value['credit'];
                        $value->balance = $update_balance;
                        $value->update();
                    }else { //Update Next Index
                        if(!isset($update_balance)){
                            ...
                            $update_balance = $update_balance + $value['debit'] - $value['credit'];
                            $value->balance = $update_balance;
                            $value->update();
                        }
                    }
        ...
        if($account_type == 3){
            ...
            $chart_of_account_bank_ending_balance = \backend\models\FinanceChartOfAccountAmount::find()
                ->where(['account_type'=>$account_type, 'chart_of_account'=>$chart_of_account])
                ->one();
            if(!empty($chart_of_account_bank_ending_balance)){
                $chart_of_account_bank_ending_balance->ending_balance = $chart_of_account_bank_amount;
                $chart_of_account_bank_ending_balance->save();
            }
        }
    ...
    public static function CheckTaskRepeat(){
        $now = date('Y-m-d');

        $mdoel_task = \backend\models\Task::find()->where(['>', 'id_repeater', 1])->andWhere(['is_main'=>1])->all();


        $sql = "UPDATE finance_customer_transaction set status = 3 where due_date < '$now' and status NOT IN (0,4) and id_transaction_type = 1";
    ...
    public function GenerateDocumentID($tbl, $type_id){

        $format = \backend\models\DocumentFormat::find()->where(['id_type'=>$type_id])->one();
        $date = date('ym');
