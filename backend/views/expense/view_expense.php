...
$this->title = "Expense No. #".$model->doc_id;
?>
    ...
    <div class="panel" id="print-content">
        <div class="panel-body">
                ...
                <div style="padding-top: 15px;">
                    <div class="btn-group dropdown pull-right no-print" style="z-index: 100;">
                        ...
                        <ul class="dropdown-menu dropdown-menu-right">
                            <li><a href="#" value="<?= Url::to(['expense/update', 'id' => $model->id]); ?>" data-title="Update Expense : <?= $model->doc_id ?>"  id='modalButton'>Update Expense</a></li>
                            ...
                            <li class="divider"></li>
                            <li><a class="button-void" data-value="<?= yii\helpers\Url::to(['expense/void-expense', 'id' => $model->doc_id]); ?>" title="VOID" data-confirm="Are you sure want to void this file?" data-method="post">Void</a></li>
                            <?php } ?>
                        </ul>
                    </div>
                    <h3 class="h1 text-uppercase text-center text-thin mar-no text-primary"><?= $model->is_bill == 1 ? "bill voucher" : "expense voucher" ?></h3>
                </div>
            </div>

            <div class="invoice-bill row">
                <div class="col-sm-6 col-xs-6 text-xs-center">
                    <strong class="text-main text-uppercase" style="font-size: 16px;"><?= $model->is_bill == 1 ? "bill" : "expense" ?> TO</strong><br>
                    ...
                    <table class="table table-bordered invoice-summary">
                        <thead>
                            <tr class="bg-trans-dark">
                                <th class="text-uppercase min-col text-left">Chart of Account</th>
                                <th class="text-uppercase min-col">Description</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <strong><?= $value->chartOfAccount->accountType->names; ?></strong>
                                    <span><?= $value->chartOfAccount->number_code .' | '. $value->chartOfAccount->names ?></span>
                                </td>
            ...
            <div class="well well-sm">
                ...
                <p><?= nl2br($model->is_bill == 0 ? \backend\models\CompanyProfile::findOne(1)->info_expense : \backend\models\CompanyProfile::findOne(1)->info_bill) ?></p>
            </div>

            <?php if($model->is_bill == 1){ ?>
                ...
                <?php if($model->paid_amount > 0){ ?>
                    ...
                    <table class="table table-condensed">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?= "<a target='_blank' href='". URL::toRoute(['expense/view','id'=>$value->payBill->mainTable->id])."' class='btn-link'> ".$value->payBill->doc_id."</a>"; ?></td>
                                <td><?= $value->payBill->id_payment_method == 3 ? $value->payBill->paymentMethod->names . ' (No. '.$value->payBill->cheque_number .' | Clear Date ' . date_format(date_create($value->payBill->clearing_date),'F d, Y') .')' : $value->payBill->paymentMethod->names ?></td>
                                <td><?= date_format(date_create($value->payBill->date),'F d, Y') ?></td>
