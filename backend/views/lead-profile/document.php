...
$this->title = 'Lead Document'; 
$this->params['breadcrumbs'][] = ['label' => 'Document']; 
$folder_data = ArrayHelper::map(\backend\models\Folder::find()
	->where(['status' => 1])
	->andWhere(['id_lead_profile' => $id_lead_profile])
	->all(), 'id', 'folder_name'); 
 
$url = Url::toRoute(['lead-profile/document']);
...
<?php
	...
	Modal::end(); 
?>
<div class="lead-profile-document">
			...
			<div class="fixed-sm-300 pull-sm-left file-sidebar">
				<div class="file-toolbar bord-btm">
					<div class="btn-file-toolbar">
						<a class="btn btn-icon add-tooltip" href="#" data-original-title="New Folder" data-toggle="tooltip" id="modalButton" value="<?= Url::toRoute(['lead-profile/create-folder','id_lead_profile'=>$id_lead_profile]) ?>"><i class="icon-2x demo-pli-folder"></i></a>
...
var id_lead_profile_current = "'.$id_lead_profile.'";
var id_lead_profile_local = localStorage.getItem("id_lead_profile_local",id_lead_profile_local);

var id_folder_current = $(".folder_wrap").attr("data-id");
var id_folder_local = localStorage.getItem("id_folder_local",id_folder_local);

if(id_lead_profile_local == id_lead_profile_current){
	id_lead_profile = id_lead_profile_current; 
...
if(id_folder_current !=null){
	$(".newFile").attr("value", base_url+"/lead-profile/upload-document?folder="+id_folder+"&id_lead_profile='.$id_lead_profile.'");
	$(".renameFolder").attr("value", base_url+"/lead-profile/rename-folder?id="+id_folder);
	$(".viewFolder").attr("value", base_url+"/lead-profile/get-info?id="+id_folder);
	$(".deleteFolder").attr("href", base_url+"/lead-profile/delete-folder?id="+id_folder);
}else{
...
documentByFolder(id_folder,id_lead_profile);
...
$(".folder_wrap").click(function(){
	...
	localStorage.setItem("id_lead_profile_local",id_lead_profile);
    documentByFolder(id_folder,id_lead_profile);
	
});

function documentByFolder(id_folder,id_lead_profile){
	$.ajax({
        url: base_url+"/lead-profile/dependent-folder",
        type: "post",
        data: {
			id_folder: id_folder,
			id_lead_profile:id_lead_profile,
			...
			{   
				...
				<li> 
					<div class="file-settings">
						<a href="${base_url}/lead-profile/download?id=${value.id}"><i style="color: #283593;" class="icon-2x demo-pli-download-from-cloud"></i></a>
						|
						<a href="${base_url}/lead-profile/remove?id=${value.id}" data-confirm="Are you sure you want to remove it permantly?"><i style="color: #c62828;" class="icon-2x demo-pli-recycling"></i></a>
					</div>
					<div class="file-attach-icon"></div>
					<a href="#" class="file-details">
						<div class="media-block">
...
} //end function documentByFolder(id_folder,id_lead_profile);
...
//==== End Ajax query data by document name
        ...
        $("#select_page_size").change(function(){
            ...
            window.location.replace(url+"?r="+controller_id+"/index&id_lead_profile='.$id_lead_profile.'&page_size="+page_size); 
        });
