    ...
    <div class="row" style="margin-top:10px;margin-bottom:10px;">
        ...
        <div class="col-md-2"> 
            <?= $form->field($model, 'id_supplier_profile_type')->widget(Select2::classname(), [
                    ...
                    'theme' => Select2::THEME_DEFAULT,
                    'language' => 'eg',
                    'options' => ['placeholder' => 'Supplier Type'],
                    'pluginOptions' => [
                        'allowClear' => true,
        ...
        <div class="col-md-4">
            <div class="pull-right">
                ...
                <button type="button" data-target="#demo-default-modal" data-toggle="modal" class="btn btn-warning btn-rounded" id="modalButton" value="<?= Url::toRoute(['supplier-profile/create']) ?>">Add New <i class="fa fa-plus-square go-right"></i>
                </button> 
            </div>
        </div>
    </div>
...
$script = <<<JS
    ...
    $('#supplierprofilesearch-from_date, #supplierprofilesearch-to_date').datepicker({
        format: 'yyyy-mm-dd',
        changeMonth: true,
        changeYear: true,
        autoclose: true,
    ...
    $("#supplierprofilesearch-from_date").change(function(){

        $('#supplierprofilesearch-to_date').datepicker('setStartDate', $(this).val());

        if(Date.parse($(this).val()) > Date.parse($('#supplierprofilesearch-to_date').val())){
            $('#supplierprofilesearch-to_date').datepicker('setDate', $(this).val());
        }

    });

    $("#supplierprofilesearch-from_date,#supplierprofilesearch-to_date").change(function(){
        $('#supplierprofilesearch-date_range').val('custom').trigger('change');
    });

    $("#supplierprofilesearch-date_range").change(function(){
        var name = $(this).val();

        if(name == 'today'){
            $("#supplierprofilesearch-from_date").val("$today");
            $("#supplierprofilesearch-to_date").val("$today");
        }else if(name == 'week'){
            $("#supplierprofilesearch-from_date").val("$week_start");
            $("#supplierprofilesearch-to_date").val("$week_end");
        }else if(name == 'month'){
            $("#supplierprofilesearch-from_date").val("$month_start");
            $("#supplierprofilesearch-to_date").val("$month_end");
        }else if(name == 'year'){
            $("#supplierprofilesearch-from_date").val("$year_start");
            $("#supplierprofilesearch-to_date").val("$year_end");
        }
    });
