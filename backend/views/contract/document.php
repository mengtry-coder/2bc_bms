...
$folder_data = ArrayHelper::map(\backend\models\Folder::find()
	->where(['status' => 1])
	->andWhere(['id_contract' => $id_contract])
	->all(), 'id', 'folder_name'); 
 
$url = Url::toRoute(['contract/document']);
							...
							<div class="fixed-sm-300 pull-sm-left file-sidebar">
								<div class="file-toolbar bord-btm">
									<div class="btn-file-toolbar">
										<a class="btn btn-icon add-tooltip" href="#" data-original-title="New Folder" data-toggle="tooltip" id="modalButton" value="<?= Url::toRoute(['contract/create-folder','id_contract'=>$id_contract]) ?>"><i class="icon-2x demo-pli-folder"></i></a>
...
var id_contract_current = "'.$id_contract.'";
var id_contract_local = localStorage.getItem("id_contract_local",id_contract_local);

var id_folder_current = $(".folder_wrap").attr("data-id");
var id_folder_local = localStorage.getItem("id_folder_local",id_folder_local);

if(id_contract_local == id_contract_current){
	id_contract = id_contract_current; 
...
if(id_folder_current !=null){
	$(".newFile").attr("value", base_url+"/contract/upload-document?folder="+id_folder+"&id_contract='.$id_contract.'");
	$(".renameFolder").attr("value", base_url+"/contract/rename-folder?id="+id_folder);
	$(".viewFolder").attr("value", base_url+"/contract/get-info?id="+id_folder);
	$(".deleteFolder").attr("href", base_url+"/contract/delete-folder?id="+id_folder);
}else{
...
documentByFolder(id_folder,id_contract);
...
$(".folder_wrap").click(function(){
	...
	localStorage.setItem("id_contract_local",id_contract);
    documentByFolder(id_folder,id_contract);
	
});

function documentByFolder(id_folder,id_contract){
	$.ajax({
        url: base_url+"/contract/dependent-folder",
        type: "post",
        data: {
			id_folder: id_folder,
			...
			{   
				...
				<li> 
					<div class="file-settings">
						<a href="${base_url}/contract/download?id=${value.id}"><i style="color: #283593;" class="icon-2x demo-pli-download-from-cloud"></i></a>
						|
						<a href="${base_url}/contract/remove?id=${value.id}" data-confirm="Are you sure you want to remove it permantly?"><i style="color: #c62828;" class="icon-2x demo-pli-recycling"></i></a>
					</div>
					<div class="file-attach-icon"></div>
					<a href="#" class="file-details">
						<div class="media-block">
...
} //end function documentByFolder(id_folder,id_contract);
...
//==== End Ajax query data by document name
        ...
        $("#select_page_size").change(function(){
            ...
            window.location.replace(url+"?r="+controller_id+"/index&id_contract='.$id_contract.'&page_size="+page_size); 
        });
