...
<div class="standard-operation-procedure-set-up-index">
    ...
    <div class="panel"> 
        ...
        <?php
            Modal::begin([
                'header' => ' <h4 class="modal-title">'.'Standard Operation Procedure Set Ups'.'</h4>',
                'id' => 'modal',
                'class' => 'modal fade',
                'size' => 'modal-lg',
            ]);
            ...
            <?= GridView::widget([
                ...
                [
                    ...
                    'value' => function ($model) {
                        $arr = '';
                        $i = 0;
                        foreach ($model->position as $key => $value) {
                            $arr = $arr .', '. $value->positionName->position_type;
                            if (++$i == 4) break;
                        }
                        return substr($arr, 2);
                    },
...
$this->registerJs('
    ...
    ');
$project = URL::toRoute(['standard-operation-procedure-set-up/sop-setup']);
$script = <<<JS

    $("td").click(function (e) {
        var id = $(this).closest("tr").data("id"); 
        var url = "$project" + '?id=' + id+'&smMenu=1';
        if(e.target == this)
            location.href = url;
    });
