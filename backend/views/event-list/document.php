...
$folder_data = ArrayHelper::map(\backend\models\Folder::find()
	->where(['status' => 1])
	->andWhere(['id_event_list' => $id_event_list])
	->all(), 'id', 'folder_name'); 
 
$url = Url::toRoute(['event-list/event-document']);
...
<?php
	...
	Modal::end(); 
?>
<div class="event-list-event-document">
	...
	<div class="panel" style="border-top-left-radius: 0px;">
		<div class="panel-body">  
            <p class="text-main text-bold">Event Document</p>
            <hr>
					...
					<div class="fixed-sm-300 pull-sm-left file-sidebar">
						<div class="file-toolbar bord-btm">
							<div class="btn-file-toolbar">
								<a class="btn btn-icon add-tooltip" href="#" data-original-title="New Folder" data-toggle="tooltip" id="modalButton" value="<?= Url::toRoute(['event-list/create-folder','id_event_list'=>$id_event_list]) ?>"><i class="icon-2x demo-pli-folder"></i></a>
						...
						?> 
							<!-- <div class="dropdown pull-right btn_folder_action">
								...
								<ul class="dropdown-menu dropdown-menu-right dropdownHieght" style="z-index:9999;overflow-x:visible; overflow-y:visible;">
									<li><a href="#" id="modalButtonGetInfo" value="'.Url::toRoute(['event-list/get-info','id'=>$key]).'">Get Info</a></li>
									<li><a href="#" id="modalButtonRenameFolder" value="'.Url::toRoute(['event-list/rename-folder','id'=>$key]).'">Rename</a></li> 
									<li class="divider"></li>
									<li><a href="'.Url::toRoute(['event-list/delete-folder','id'=>$key]).'" onclick="confirmDelete()">Delete</a></li> 
								</ul>
							</div>  -->
						</div> 
...
var id_event_list_current = "'.$id_event_list.'";
var id_event_list_local = localStorage.getItem("id_event_list_local",id_event_list_local);

var id_folder_current = $(".folder_wrap").attr("data-id");
var id_folder_local = localStorage.getItem("id_folder_local",id_folder_local);

if(id_event_list_local == id_event_list_current){
	id_event_list = id_event_list_current; 
...
if(id_folder_current !=null){
	$(".newFile").attr("value", base_url+"/event-list/upload-document?folder="+id_folder+"&id_event_list='.$id_event_list.'");
	$(".renameFolder").attr("value", base_url+"/event-list/rename-folder?id="+id_folder);
	$(".viewFolder").attr("value", base_url+"/event-list/get-info?id="+id_folder);
	$(".deleteFolder").attr("href", base_url+"/event-list/delete-folder?id="+id_folder);
}else{
...
documentByFolder(id_folder,id_event_list);
...
$(".folder_wrap").click(function(){
	...
	localStorage.setItem("id_event_list_local",id_event_list);
    documentByFolder(id_folder,id_event_list);
	
});

function documentByFolder(id_folder,id_event_list){
	$.ajax({
        url: base_url+"/event-list/dependent-folder",
        type: "post",
        data: {
			id_folder: id_folder,
			...
			{   
				...
				<li> 
					<div class="file-settings">
						<a href="${base_url}/event-list/download?id=${value.id}"><i style="color: #283593;" class="icon-2x demo-pli-download-from-cloud"></i></a>
						|
						<a href="${base_url}/event-list/remove?id=${value.id}" data-confirm="Are you sure you want to remove it permantly?"><i style="color: #c62828;" class="icon-2x demo-pli-recycling"></i></a>
					</div>
					<div class="file-attach-icon"></div>
					<a href="#" class="file-details">
						<div class="media-block">
...
} //end function documentByFolder(id_folder,id_event_list);
