...
$url = Url::toRoute(['project/document']);
							...
							<div class="fixed-sm-300 pull-sm-left file-sidebar">
								<div class="file-toolbar bord-btm">
									<div class="btn-file-toolbar">
										<a class="btn btn-icon add-tooltip" href="#" data-original-title="New Folder" data-toggle="tooltip" id="modalButton" value="<?= Url::toRoute(['project/create-folder','id_project'=>$id_project]) ?>"><i class="icon-2x demo-pli-folder"></i></a>
...
var id_project_current = "'.$id_project.'";
var id_project_local = localStorage.getItem("id_project_local",id_project_local);

var id_folder_current = $(".folder_wrap").attr("data-id");
var id_folder_local = localStorage.getItem("id_folder_local",id_folder_local);

if(id_project_local == id_project_current){
...
if(id_folder_current !=null){
	$(".newFile").attr("value", base_url+"/project/upload-document?folder="+id_folder+"&id_project='.$id_project.'");
	$(".renameFolder").attr("value", base_url+"/project/rename-folder?id="+id_folder);
	$(".viewFolder").attr("value", base_url+"/project/get-info?id="+id_folder);
	$(".deleteFolder").attr("href", base_url+"/project/delete-folder?id="+id_folder);
}else{
...
documentByFolder(id_folder,id_project);
...
$(".folder_wrap").click(function(){
	...
	localStorage.setItem("id_project_local",id_project);
    documentByFolder(id_folder,id_project);
	
});

function documentByFolder(id_folder,id_project){
	$.ajax({
        url: base_url+"/project/dependent-folder",
        type: "post",
        data: {
			id_folder: id_folder,
			...
			{   
				...
				<li> 
					<div class="file-settings">
						<a href="${base_url}/project/download?id=${value.id}"><i style="color: #283593;" class="icon-2x demo-pli-download-from-cloud"></i></a>
						|
						<a href="${base_url}/project/remove?id=${value.id}" data-confirm="Are you sure you want to remove it permantly?"><i style="color: #c62828;" class="icon-2x demo-pli-recycling"></i></a>
					</div>
					<div class="file-attach-icon"></div>
					<a href="#" class="file-details">
						<div class="media-block">
...
} //end function documentByFolder(id_folder,id_project);
