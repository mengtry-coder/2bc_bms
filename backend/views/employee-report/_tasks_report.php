...
$taskStatus = ArrayHelper::map(\backend\models\TaskStatus::find()
    ->all(), 'id', function($model)
    {
        return $model->name;
    });
...
<div class="task-form report">

    <?php $form = ActiveForm::begin([
        'action' => ['tasks','smMenu'=>1],
        'method' => 'post',
        'id' => 'idTask',
    ]); ?>
                ...
                <br>
                    <table class="table table-condensed">
                        ...
                        <tr>
                            <td colspan="10">
                                <?php
                                    $check_list = \backend\models\TaskCheckList::findAll(['id_task'=>$n['id']]);
                                    if(!empty($check_list)){
                                        $i = 1;
                                      foreach ($check_list as $list) {
                                        $status = $list->status == 1 ? ' (Completed)' : ' (Processing)';
                            ?>
                                          <p style="padding-left: 10rem; font-style: italic;" class="text-muted"><?= $i++ .". ".  $list->description . $status; ?></p>
                            <?php
                                      }
                                    }
                                ?>
