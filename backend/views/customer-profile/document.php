...
$url = Url::toRoute(['customer-profile/document']);
...
<?php
	...
	Modal::end(); 
?>
<div class="customer-profile-document">
			...
			<div class="fixed-sm-300 pull-sm-left file-sidebar">
				<div class="file-toolbar bord-btm">
					<div class="btn-file-toolbar">
						<a class="btn btn-icon add-tooltip" href="#" data-original-title="New Folder" data-toggle="tooltip" id="modalButton" value="<?= Url::toRoute(['customer-profile/create-folder','id_customer_profile'=>$id_customer_profile]) ?>"><i class="icon-2x demo-pli-folder"></i></a>
...
if(id_folder_current !=null){
	$(".newFile").attr("value", base_url+"/customer-profile/upload-document?folder="+id_folder+"&id_customer_profile='.$id_customer_profile.'");
	$(".renameFolder").attr("value", base_url+"/customer-profile/rename-folder?id="+id_folder);
	$(".viewFolder").attr("value", base_url+"/customer-profile/get-info?id="+id_folder);
	$(".deleteFolder").attr("href", base_url+"/customer-profile/delete-folder?id="+id_folder);
}else{
...
function documentByFolder(id_folder,id_customer_profile){
	$.ajax({
        url: base_url+"/customer-profile/dependent-folder",
        type: "post",
        data: {
			id_folder: id_folder,
			...
			{   
				...
				<li> 
					<div class="file-settings">
						<a href="${base_url}/customer-profile/download?id=${value.id}"><i style="color: #283593;" class="icon-2x demo-pli-download-from-cloud"></i></a>
						|
						<a href="${base_url}/customer-profile/remove?id=${value.id}" data-confirm="Are you sure you want to remove it permantly?"><i style="color: #c62828;" class="icon-2x demo-pli-recycling"></i></a>
					</div>
					<div class="file-attach-icon"></div>
					<a href="#" class="file-details">
						<div class="media-block">
