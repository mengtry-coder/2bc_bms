...
<script>
    $(document).ready(function(){
        ...
        while(current_page.parent().parent().hasClass('active-sub active')) {
            current_page.parent().parent().addClass('active-link');
            current_page = current_page.parent().parent();
        }
    });
</script>
...
function Navigation(){
    ...
    $command = Yii::$app->db->createCommand("
        select 
            ...
            b.css_icon,
            b.css_class
        from user_type_permission a 
            inner join permission b on a.permission_id = b.id  
    ...
    foreach ($data as $m) {
        ...
        $url = $m["controller"].'/'.$m["action"];
        $urls = $asset_path."/".$url;
        $parent_id = $m["id"];
        $has_children = IsHasChildren($user_type_id, $m["id"])>0;
        $css_icon = $m["css_icon"];
        $id = "class=".$m["controller"].'-'.$m["action"];
        $css_has_children = "";
        if($has_children !=""){
            $css_has_children = "class='active-sub active'";
            $id = '';
        }
        echo "<li  $id>
            <a href='$urls'><i class=".$css_icon."></i> <span class='menu-title' >$menu_text</span></a>";
        SubNavigation($parent_id,$has_children);
        echo "</li>";
    }
}
function IsHasChildren($user_type_id, $permission_id){
$sql="
    select 
        count(x.id)  
    from user_type_permission x 
    inner join permission y on x.permission_id = y.id 
    and x.user_type_id = $user_type_id 
    and coalesce(y.parent_id,0) = $permission_id 
    and coalesce(y.show_in_menu,0)
";

$command = Yii::$app->db->createCommand($sql);
$command->bindParam(":user_type_id", $user_type_id);
$command->bindParam(":permission_id", $permission_id);
return $command->queryScalar();
}
    function SubNavigation($parent_id, $is_has_children)
{
$asset_path = Yii::getAlias('@web');
$user_id = Yii::$app->user->getId();
$user = User::findOne($user_id);
...
$command = Yii::$app->db->createCommand("
    ...
    and b.parent_id = :parent_id order by b.sort_order, b.menu_text
");
...
if(count($data)>0) {
    ...
    foreach ($data as $m) {
        ...
        echo "<li  $id>
        <a href='$urls'><span class='fa $css_icon'></span> $menu_text</a>";
        //echo Html::a('<span class="fa '.$css_icon.'"></span> <span class="xn-text">'.$menu_text.'</span>', [$url]);
        SubNavigation($m["id"],$has_children);
        echo "</li>";
    }
    echo "</ul>";
}
