...
$search_content = "Proposal Number, Customer, Subject, Amount";
...
<div class="proposal-search">
    ...
    <?php $form = ActiveForm::begin([
        'action' => ['index','id_customer_profile'=>$id_customer_profile,'id_lead_profile'=>$id_lead_profile,'smMenu'=>$smMenu],
        'method' => 'get',
    ]); ?>
    ...
    <div class="row">
        <div class="col-md-9">
            <div class="row">
                <div class="col-md-2">
                    <?= $form->field($model, 'date_type')->dropDownList($date_type, ['style'=>'margin-top: 10px;'])->label(false) ?>
                </div>
                <div class="col-md-2">
        ...
        <div class="col-md-3">
            <div class="pull-right" style="margin-top:10px;">
                ...
                <button type="button" data-title="New Proposal" data-target="#demo-default-modal" data-toggle="modal" class="btn btn-warning btn-rounded" id="modalButton" value="<?= Url::toRoute(['create']) ?>"> New Proposal <i class="fa fa-plus-square go-right"></i> 
                </button>
            </div>
        </div>
    </div>
...
$script = <<<JS
    ...
    $('#proposalsearch-from_date, #proposalsearch-to_date').datepicker({
        format: 'yyyy-mm-dd',
        changeMonth: true,
        changeYear: true,
        autoclose: true,
    ...
    $("#proposalsearch-from_date").change(function(){

        $('#proposalsearch-to_date').datepicker('setStartDate', $(this).val());

        if(Date.parse($(this).val()) > Date.parse($('#proposalsearch-to_date').val())){
            $('#proposalsearch-to_date').datepicker('setDate', $(this).val());
        }

    });

    $("#proposalsearch-from_date,#proposalsearch-to_date").change(function(){
        $('#customerinvoicesearch-date_range').val('custom').trigger('change');
    });

    $("#proposalsearch-date_range").change(function(){
        var name = $(this).val();

        if(name == 'today'){
            $("#proposalsearch-from_date").val("$today");
            $("#proposalsearch-to_date").val("$today");
        }else if(name == 'week'){
            $("#proposalsearch-from_date").val("$week_start");
            $("#proposalsearch-to_date").val("$week_end");
        }else if(name == 'month'){
            $("#proposalsearch-from_date").val("$month_start");
            $("#proposalsearch-to_date").val("$month_end");
        }else if(name == 'year'){
            $("#proposalsearch-from_date").val("$year_start");
            $("#proposalsearch-to_date").val("$year_end");
        }
    });
