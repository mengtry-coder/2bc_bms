...
$proposal_status = ArrayHelper::map(\backend\models\ProposalStatus::find()
    ->all(), 'id', function($model){return $model->name;});
$currency = ArrayHelper::map(\backend\models\Currency::find()
    ->all(), 'id', function($model){return $model->name;});
$related_to = ArrayHelper::map(\backend\models\ProposalRelatedTo::find()
    ->all(), 'id', 'name');
...
<div class="proposal-form" id="container">
    ...
    <div class="row">
        <div class="col-md-12">
          <div class="col-md-3">
            ...
            ?>
            <?= $form->field($model, 'id_lead_customer')->widget(Select2::classname(), [
                    'data' => $customer,
                    'theme' => Select2::THEME_DEFAULT,
                    'language' => 'eg',
                    'options' => ['placeholder' => 'Select'],
...
<!-- <input id="demo-sw-checked" type="checkbox" checked> -->
                    ...
                    <td>
                        <div class="input-group">
                            ...
                            <div class="input-group-btn">
                                <button type="button" class="btn btn-default dropdown-toggle btn-primary" id="label-discount" data-toggle="dropdown">% <span class="caret"></span></button>
    ...
    <div class="modal-footer">
        <div class="text-right">
            <?= Html::submitButton('Convert To Quotation', ['class' => 'btn btn-success btn-rounded']) ?>
        </div>
    </div>
...
$script = <<<JS
    ...
    $('#proposal-date, #proposal-open_till').datepicker({
        format: 'yyyy-mm-dd',
        changeMonth: true,
        changeYear: true,
        autoclose: true,
    ...
    $("#proposal-related_id").change(function(){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                $('#quotation-id_project-container').html(str);
            }
        });

    });

    $("#proposal-id_lead_customer").change(function(){
        // var text = $(this).select2('data')[0]['text'];
        // $(".field-task-id_related_profile label").text(text);
        var id = $(this).val();
        $.ajax({
            url: base_url+'/proposal/dependent-get-customer-info',
            type: 'post',
            data: {
                id: id,
                action: 'get_profile'
            ...
            success: function(response){
                var data = JSON.parse(response);
                $('#proposal-address').val(data['address']);
                $('#proposal-email').val(data['email_address']);
                $('#proposal-phone').val(data['phone_number']);
                $('#proposal-city').val(data['city']);
                $('#proposal-country').val(data['country']);
                $('#proposal-to').val(data['company_name']);
                $('#proposal-zip_code').val(data['zip_code']);
            }
        });
