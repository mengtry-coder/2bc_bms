    ...
    <div class="row" style="margin-bottom:10px;">
        ...
        <div class="col-md-3">
            <div class="pull-right" style="margin-top: 10px;">
                ...
                <a data-target="#demo-default-modal" data-toggle="modal" class="btn btn-warning btn-rounded" id="modalButton" value="<?= Url::toRoute(['finance-opening-balance/create']) ?>">Add New <i class="fa fa-plus-square go-right"></i>
                </a>  
            </div>
        </div>
    </div>
...
$script = <<<JS
    ...
    $('#financeopeningbalancesearch-from_date, #financeopeningbalancesearch-to_date').datepicker({
        format: 'yyyy-mm-dd',
        changeMonth: true,
        changeYear: true,
        autoclose: true,
    ...
    $("#financeopeningbalancesearch-from_date").change(function(){

        $('#financeopeningbalancesearch-to_date').datepicker('setStartDate', $(this).val());

        if(Date.parse($(this).val()) > Date.parse($('#financeopeningbalancesearch-to_date').val())){
            $('#financeopeningbalancesearch-to_date').datepicker('setDate', $(this).val());
        }

    });

    $("#financeopeningbalancesearch-from_date,#financeopeningbalancesearch-to_date").change(function(){
        $('#financeopeningbalancesearch-date_range').val('custom').trigger('change');
    });

    $("#financeopeningbalancesearch-date_range").change(function(){
        var name = $(this).val();

        if(name == 'today'){
            $("#financeopeningbalancesearch-from_date").val("$today");
            $("#financeopeningbalancesearch-to_date").val("$today");
        }else if(name == 'week'){
            $("#financeopeningbalancesearch-from_date").val("$week_start");
            $("#financeopeningbalancesearch-to_date").val("$week_end");
        }else if(name == 'month'){
            $("#financeopeningbalancesearch-from_date").val("$month_start");
            $("#financeopeningbalancesearch-to_date").val("$month_end");
        }else if(name == 'year'){
            $("#financeopeningbalancesearch-from_date").val("$year_start");
            $("#financeopeningbalancesearch-to_date").val("$year_end");
        }
    });
