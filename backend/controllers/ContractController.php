...
use backend\models\ContractData;
...
class ContractController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if($id_customer_profile != 0){
            ...
            $dataProvider->query->where(['=','contract.id_customer',$id_customer_profile]);
        }else{
            $searchModel = new ContractSearch();
            $dataProvider = $searchModel->search(Yii::$app->request->queryParams); 
        }
    ...
    public function actionValidation($id = null){
        ...
        $model = $id === null ? new Contract : Contract::findOne($id);
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->doc_id = (new GlobalFunction)->GenerateDocumentID('contract', 9);
            ...
            $other_charge = Yii::$app->request->post('other_charge');
            ...
            try
            {
                ...
                $customer_contact = Yii::$app->request->post('customer_contact');

                if((!empty($customer_contact)) || ($customer_contact != '')){
                    foreach ($customer_contact as $key) {
                        $data = new \backend\models\ContractCustomerContact();
                        $data->id_contract = $model->id;
                        $data->id_customer_contact = $key;
                        if(!$data->save()) throw new Exception("Failed to save data!");
                    }
                }
    ...
    public function actionUpdate($id)
    {
        ...
        $model_item = ContractData::findAll(['id_contract' => $id]);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            try
            {
                ...
                \backend\models\ContractCustomerContact::deleteAll(['id_contract'=>$model->id]);
                if((!empty($customer_contact)) || ($customer_contact != '')){
                    foreach ($customer_contact as $key) {
                        $data = new \backend\models\ContractCustomerContact();
                        $data->id_contract = $model->id;
                ...
                \backend\models\ContractData::deleteAll(['id_contract'=>$model->id]);
                if(!empty($item_name)){
                    foreach ($item_name as $key => $value) {
                        $data = new ContractData();
                        $data->id_contract = $model->id;
    ...
    {
        $model = $this->findModel($id);
        $arr = (new \common\models\GlobalFunction)->getColumnListAsArray('id_contract','contract_id');
    ...
    public function actionDependent(){
        ...
        if(Yii::$app->request->isAjax){
            ...
            if(Yii::$app->request->post('action') =='get_item_data'){
                ...
                $description = \backend\models\ItemGroup::find()->where(['id'=>$id])->asArray()->one();
                $item = \backend\models\Item::find()->where(['id_group'=>$id])->asArray()->all();
                $response = ['description'=>$description, 'item'=>$item];
                return json_encode($response);
            }

            if(Yii::$app->request->post('action') =='get_customer_contact'){
                $id = Yii::$app->request->post('id');

                $response = \backend\models\CustomerProfileContact::find()->where(['id_customer'=>$id])->asArray()->all();
    ...
    public function actionDocument($id_contract)
    {
        $current_page = Yii::$app->controller->id."-index";
        $this->view->params['current_page'] = $current_page;
        $this->layout="contract_layout";
    ...
    public function actionUpload($folder,$id_contract)
    {
            ...
            if (isset($_FILES[$fileName]))
            {
                ...
                if (isset($file)) {
                    ...
                    elseif(
                        ...
                        $model_file->id_contract = $id_contract;
                        $model_file->status = 1;
    ...
    public function actionCreateFolder()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            FROM folder
            WHERE folder.id_contract = "'.$model->id_contract.'"
    ...
    public function actionDependentFolder(){
        ...
        if($request->isAjax){
            if($request->post('action')=='get_data_folder'){
                ...
                $id_contract = $request->post('id_contract');
                ...
                and document.id_contract = :id_contract
                ')->bindParam(':id_contract',$id_contract)->bindParam(':id_folder',$id_folder)->queryAll();
                return json_encode($response);
            }
