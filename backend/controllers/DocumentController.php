...
class DocumentController extends Controller
{
    ...
    public function actionIndex($document_permission = 1)
    {
        $current_page = Yii::$app->controller->id."-index";
        $this->view->params['current_page'] = $current_page;
        
        $searchModel = new DocumentSearch();
        // $searchModel->document_permission = $document_permission; 
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        $dataProvider->query->where(['document_permission'=>$document_permission]);
    ...
    public function actionSubscriber($document_permission = 2)
    {
    ...
    public function actionUpload($folder,$document_permission)
    {
            ...
            if (isset($_FILES[$fileName]))
            {
                ...
                if (isset($file)) {
                    ...
                    elseif(
                        ...
                        $model_file->document_permission = $document_permission;
                        $model_file->status = 1;
    ...
    //==== Start Folder block ====//
    public function actionCreateFolder($document_permission)
    {
        $model = new Folder();

        $model->created_date =  date('Y-m-d H:i:s');
        $model->created_by = Yii::$app->user->getId(); 
        $model->id_document_permission = $document_permission; 
        if ($model->load(Yii::$app->request->post())) {
            $check_folder_name = Yii::$app->db->createCommand('
            SELECT folder.folder_name as folder_name
            FROM folder
            WHERE folder.id_document_permission = "'.$model->id_document_permission.'"
    ...
    public function actionDependent(){
        ...
        if($request->isAjax){
            if($request->post('action')=='get_data_folder'){
                ...
                $document_permission = $request->post('document_permission');
                ...
                and document.document_permission = :document_permission
                ')->bindParam(':document_permission',$document_permission)->bindParam(':id_folder',$id_folder)->queryAll();
